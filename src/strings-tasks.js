...
function sumOfCodes(str) {
  return !isString(str)
    ...
    : str
        .split('')
        .map((item, index) => str.charCodeAt(index))
...
function countVowels(str) {
  ...
  return str.split('').filter((letter) => vowels.includes(letter.toLowerCase()))
    .length;
}
...
function isPalindrome(str) {
  ...
  const string = str
    .split('')
    .filter((item) => !nonLetters.includes(item))
    .join('');
...
function invertCase(str) {
  return str
    .split('')
    .map((item) => (item < 'a' ? item.toLowerCase() : item.toUpperCase()))
    .join('');
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((item) =>
      alphabet.includes(item) ? alphabet[alphabet.indexOf(item) + 13] : item
    )
    .join('');
}
